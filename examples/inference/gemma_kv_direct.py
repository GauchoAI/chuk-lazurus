...
def main():
    ...
    kv_gen_mod  = _load("chuk_lazarus.inference.kv_generator",  _inf / "kv_generator.py")
    ...
    # Memory accounting
    per_token_bytes = kv_gen.kv_bytes(1) // config.num_hidden_layers  # wrong, fix
    ...
    if kv_bytes == rs_bytes:
        print(f"    ✓ KV-direct == RS residual == standard KV (hidden = 2×nkv×head_dim)")
    else:
        ratio = rs_bytes / kv_bytes
        print(f"    KV-direct {ratio:.2f}× smaller than RS residual "
    ...
    if not args.skip_rs:
        ...
        results["rs"] = rs_best
        print(f"  {YELLOW}RS  best: {args.gen_tokens/rs_best:.1f} tok/s  "
    ...
    print(f"  {CYAN}KVD best: {args.gen_tokens/kvd_best:.1f} tok/s  "
    ...
    if kv_toks and kvd_toks:
        ...
        if match:
            ...
            print(f"  ✗ Output mismatch (expected — generation is stochastic/greedy differs at first divergence)")
    ...
    if rs_toks and kvd_toks:
        ...
        if match:
            ...
            print(f"  ✗ KV-direct vs RS: first divergence at "
    ...
    print(f"  {'-'*44}")
    print(f"  {'Standard KV':<12} {args.gen_tokens/results['kv']:>8.1f} {'1.00×':>8} {fmt_bytes(kv_bytes):>12}")
    if "rs" in results:
        print(f"  {'RS residual':<12} {args.gen_tokens/results['rs']:>8.1f} "
              f"{kv_best/results['rs']:>7.2f}× {fmt_bytes(rs_bytes):>12}")
    print(f"  {'KV-direct':<12} {args.gen_tokens/results['kvd']:>8.1f} "
    ...
    if kvd_ratio <= 1.2:
        ...
        print(f"    Confirmed: skipping K,V recompute eliminates the RS overhead.")
    elif kvd_ratio <= 1.8:
        print(f"  {YELLOW}~ KV-direct close to KV ({kvd_ratio:.2f}× overhead){RESET}")
        print(f"    Partial win — remaining overhead from store concatenation or FFN.")
    else:
        print(f"  ✗ KV-direct still {kvd_ratio:.2f}× slower than KV")
        print(f"    Other bottleneck — investigate FFN cost or memory bandwidth.")

    if "rs" in results:
        rs_ratio = kv_best / results["rs"]
