...
def parse_args():
    ...
    p.add_argument(
        "--budget-mb", type=float, default=4.0, help="Device memory budget for KV cache in MB"
    )
    p.add_argument("--turns", type=int, default=6, help="Number of conversation turns to simulate")
    p.add_argument(
        "--tokens-per-turn", type=int, default=30, help="Synthetic input tokens per turn"
    )
    p.add_argument("--gen-tokens", type=int, default=20, help="Max tokens to generate per turn")
    p.add_argument(
        "--checkpoint-interval",
        type=int,
        default=50,
...
def main():
    ...
    print("  │  WARM : Checkpoints + dark residuals     │")
    print("  │  COLD : Token IDs (Redis, 4 bytes/tok)  │")
    print(f"  └─{'─' * 40}─┘")
...
""")
    ...
    if state.dark_residuals:
        ...
        for layer_idx, residual in sorted(state.dark_residuals.items()):
            print(
                f"  Layer {layer_idx:>2} residual: shape={tuple(residual.shape)}  "
