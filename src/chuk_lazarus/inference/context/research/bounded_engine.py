...
class ConversationState:
    ...
    All tensor shapes assume batch_size=1:
        KV mode hot_state    : list[num_layers] of (K, V) where K/V = (1, nkv, S, head_dim)
        RS mode hot_state    : list[num_layers] of (1, S, hidden_size)
        kv_direct hot_state  : same as KV mode
        checkpoints residual : (1, checkpoint_len, hidden_size)
        dark_residuals       : {layer_idx: (1, S, hidden_size)}
    """

    # --- Tier 1: hot ---
    # KV mode: list of (K, V) per layer — K/V shape (1, nkv, S, head_dim)
    # RS mode: list of (1, S, hidden_size) residual per layer
...
class BoundedKVEngine:
    ...
    def __init__(
        ...
        self._kv_gen = KVDirectGenerator.from_gemma_rs(rs_model, config)
    ...
    def _ensure_kv(self, state: ConversationState) -> tuple[PathLabel, ConversationState, Any]:
        ...
        if best is not None:
            state, out = self._warm_rebuild_kv(state, best)
            return PathLabel.WARM, state, out

        state, out = self._cold_rebuild_kv(state)
        return PathLabel.COLD, state, out

    def _warm_rebuild_kv(
        self, state: ConversationState, ckpt: Checkpoint
    ) -> tuple[ConversationState, Any]:
        """Rebuild KV cache using a warm checkpoint to reduce compute.

        Strategy: prefill the checkpoint prefix (tokens up to ckpt.token_position)
        to populate the KV cache, then incrementally extend with any uncached
        suffix tokens. The checkpoint's layer_idx tells us a good split point
        for the RS model — we run layers 0..layer_idx from the checkpoint
        residual instead of re-embedding from token IDs.

        For KV mode this still requires a full forward pass to build K,V at
        every layer, but the checkpoint provides the residual at layer_idx
        so we save layers 0..layer_idx of compute for the prefix.
        """
        # Token IDs for the prefix covered by the checkpoint
        prefix_end = ckpt.token_position - state.window_start
        prefix_ids = state.window_token_ids[:prefix_end]
        suffix_ids = state.window_token_ids[prefix_end:]

        # Prefill the prefix — use the full standard model (KV cache needs all layers)
        if prefix_ids:
            ids = mx.array(prefix_ids)[None]
            out = self.std(ids)
            mx.eval(out.logits)
            cache = out.cache
        ...
        # Incrementally extend with suffix tokens (one at a time for KV mode)
